...
- **Effort:** S
- **Files:** all of: `app/pages/index.vue:166`, `app/pages/about.vue:60`, `app/pages/howtouse.vue:28`, `app/pages/research.vue:34`, `app/components/Navbar.vue:107`, `app/components/Lpolymarker.vue:92, 363`
...
Drops one dependency (`axios` ~30 KB), removes the `app/pages/index.vue` mixed `fetch` vs `axios` inconsistency, and `useFetch` would give proper SSR-data handling for free if SSR is ever turned back on.

## Bugs

### `app/pages/research.vue` is broken — Bootstrap-Vue not installed
...
- **Files:** `tailwind.config.js:17`, `app/layouts/default.vue:58, 92-110`

`tailwind.config.js` says `primary: #009de0` (cyan). `app/layouts/default.vue` defines `--primary: #0066cc` (deeper blue) and hand-rolls `.text-primary` / `.bg-primary` rules that override Tailwind's generated utilities. Net result: `text-primary` resolves to `#0066cc`, the Tailwind value is silently dead.

Pick one. Recommended: keep Tailwind's value, set it to whichever blue is correct, delete the CSS-var override block in `app/layouts/default.vue`.

### Viewport meta declared twice; `user-scalable=no` is an a11y regression

- **Severity:** medium (accessibility)
- **Effort:** S
- **Files:** `nuxt.config.ts:15`, `app/layouts/default.vue:42-51`

Layout's `viewport` wins (later in head merge), so the config entry is dead. The layout's content is `width=device-width, initial-scale=1, viewport-fit=cover, maximum-scale=1.0, user-scalable=no` — `user-scalable=no` and `maximum-scale=1.0` block pinch-zoom. The justification is iOS overscroll, but `position: fixed` on `body` (`app/layouts/default.vue:69-72`) already kills overscroll, so these flags are unnecessary collateral damage.
...
### Defensive dead-code loader in `app/pages/index.vue:43-50`
...
Only `pt20240711_en.svg` is referenced (`app/pages/index.vue:16`). These ship in every deploy but are unused: `pt2020-02_en.svg`, `pt2020-02_en2.svg`, `pt20220515_en.svg`, `ptkort2020.svg`. Decide whether to keep for archival or delete.
...
- **Files:** `app/pages/index.vue:33-34`, `app/components/Lpolymarker.vue` (`styleFunction`)
...
- **Effort:** S
- **Files:** `app/components/Lpolymarker.vue:112, 128`
...
- **Files:** `app/components/LeafletMap.vue`, `app/components/Lpolymarker.vue:336-353`, `nuxt.config.ts`
...
- **Files:** `app/components/Lpolymarker.vue:381` (`fetchRoutes`)
...
- **Files:** `app/components/Lpolymarker.vue:31, 536-549`
...
- **Severity:** medium
- **Effort:** S
- **Files:** `app/components/AdBanner.vue:43`
...
- **Files:** `app/components/Lpolymarker.vue:9, 38`
...
- **Effort:** S
- **Files:** `app/pages/index.vue:44-50`
...
- **Severity:** low
- **Effort:** S
- **Files:** `app/assets/css/main.css:7`, `app/components/Lpolymarker.vue` (modal headings)
...
- **Severity:** low
- **Effort:** S
- **Files:** `app/components/Navbar.vue:2`, `app/assets/css/main.css` (`prose a` rules)
...
- **Severity:** low
- **Effort:** S
- **Files:** `app/components/Lpolymarker.vue:63-66`
